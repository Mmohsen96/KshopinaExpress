...
class UserSystemInfoHelper
{
  private static function get_user_agent(){
      ...
      public static function add_new_visitor($ksp){
        ...
        if ($getip !="UNKNOWN") {
            try {
                $ip_information=json_decode(file_get_contents("http://ipinfo.io/{$getip}/json?token=add6d175ef3bd5"));
            } catch (\Throwable $th) {
                DB::insert('insert into errors (shipment_number,message,system_name) values (?,?,?)', [$getip,"ipinfo problem", "Tracking new visitor"]);
            }
            ...
            try {
                ...
                DB::insert('insert into visitor (country,city,ip,internet_company,location,device,browser,os,visited_at,ksp_number) values (?,?,?,?,?,?,?,?,?,?)',
                 [$ip_information->country,$ip_information->city,$getip,$ip_information->org,$ip_information->loc,$getdevice,$getbrowser,$getos,$date,$ksp]);

            } catch (\Throwable $th) {
                DB::insert('insert into errors (shipment_number,message,system_name) values (?,?,?)', [$ksp,$getip."Problem with add data", "Tracking new visitor"]);
            }
        } else {
            DB::insert('insert into errors (message,system_name) values (?,?,?)', ["UNKNOWN IP", "Tracking new visitor"]);
        }
